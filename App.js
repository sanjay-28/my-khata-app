...
import { KNOWN_SOURCES, parseNotification, guessCategory } from "./notificationParser";

const STORAGE_KEY = "@expenses_v1";
const BUDGET_KEY = "@budget_v1";
const PENDING_KEY = "@pending_review_v1";
const LAST_SYNC_KEY = "@last_sync_v1";
// Set this to your Cloud Run / backend URL once you've deployed it (Step 2).
// Leave blank to keep everything fully on-device with no sync.
const SYNC_ENDPOINT = "";
...
export default function App() {
  ...
  const [pending, setPending] = useState([]); // captured-but-unreviewed entries
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const p = await AsyncStorage.getItem(PENDING_KEY);
  ...
  // Listen for notifications from GPay/PhonePe/PayZapp/etc (Android only,
  // requires notification-access permission granted in phone Settings).
  useEffect(() => {
    if (!hasPermission) return;
    ExpoAndroidNotificationListenerService.setAllowedPackages(Object.keys(KNOWN_SOURCES));
    const subscription = ExpoAndroidNotificationListenerService.addListener(
      ...
      (event) => {
        ...
        if (!parsed || parsed.direction !== "debit") return; // only track money going out
        setPending((prev) => [
          {
  ...
  // Auto-sync check: runs once when the app opens. If it's evening/night
  // (>= 6pm) and we haven't synced today, push a batch to your backend.
  useEffect(() => {
    (async () => {
      if (!SYNC_ENDPOINT) return;
      const lastSync = await AsyncStorage.getItem(LAST_SYNC_KEY);
      const now = new Date();
      const alreadySyncedToday = lastSync && monthKey(lastSync) + lastSync.slice(8, 10) === monthKey(todayStr()) + todayStr().slice(8, 10);
      if (now.getHours() >= 18 && !alreadySyncedToday) {
        syncToServer();
      }
    })();
  ...
  async function syncToServer() {
    if (!SYNC_ENDPOINT) {
      Alert.alert("No sync server set", "Add your Cloud Run URL to SYNC_ENDPOINT in App.js first.");
      return;
    }
    setSyncing(true);
    try {
      await fetch(SYNC_ENDPOINT + "/sync", {
        method: "POST",
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          {!hasPermission && (
            <TouchableOpacity
              ...
              onPress={() => ExpoAndroidNotificationListenerService.requestPermission()}
            >
              <Text style={styles.permBannerText}>
                Turn on notification access to auto-capture GPay/PhonePe/etc payments →
              </Text>
            </TouchableOpacity>
          )}

          {pending.length > 0 && (
            <TouchableOpacity style={styles.pendingBanner} onPress={() => setReviewVisible(true)}>
              <Text style={styles.pendingBannerText}>
                {pending.length} captured payment{pending.length > 1 ? "s" : ""} to review →
              </Text>
            </TouchableOpacity>
          )}

          {SYNC_ENDPOINT ? (
            <TouchableOpacity onPress={syncToServer} disabled={syncing} style={{ marginTop: 10 }}>
              <Text style={{ color: "#9BA89F", fontSize: 11 }}>{syncing ? "Syncing…" : "Sync now"}</Text>
            </TouchableOpacity>
          ) : null}
        </View>
      ...
      <Modal visible={reviewVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBody}>
            <Text style={styles.modalTitle}>Review captured payments</Text>
            <Text style={{ color: "#6B6A60", fontSize: 12, marginBottom: 10 }}>
              Detected from your linked apps' notifications. Approve to add, or dismiss if it's not
              a real expense (e.g. a balance-check or incoming payment).
            </Text>
            <ScrollView style={{ maxHeight: 420 }}>
              {pending.map((item) => (
                <View key={item.id} style={styles.pendingRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.rowTitleDark}>{item.note}</Text>
                    <Text style={styles.rowSubDark}>{item.source} · {item.category} · {fmt(item.amount)}</Text>
                  </View>
                  <TouchableOpacity onPress={() => approvePending(item)} style={styles.approveBtn}>
                    <Text style={{ color: "#fff", fontSize: 12, fontWeight: "600" }}>Add</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => dismissPending(item.id)} style={{ padding: 8 }}>
                    <Text style={{ color: "#B33F2B", fontSize: 12 }}>Dismiss</Text>
                  </TouchableOpacity>
                </View>
              ))}
              {pending.length === 0 && <Text style={{ textAlign: "center", color: "#8A8578", padding: 20 }}>All caught up.</Text>}
            </ScrollView>
            <TouchableOpacity style={styles.cancelBtn} onPress={() => setReviewVisible(false)}>
              <Text style={{ color: "#8A8578" }}>Close</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  permBanner: { backgroundColor: "#1B3B36", borderRadius: 10, padding: 10, marginTop: 12 },
  permBannerText: { color: "#D9A02D", fontSize: 12, fontWeight: "600" },
  pendingBanner: { backgroundColor: "#D9A02D", borderRadius: 10, padding: 10, marginTop: 10 },
  pendingBannerText: { color: "#16332F", fontSize: 12, fontWeight: "700" },
  pendingRow: { flexDirection: "row", alignItems: "center", paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "#EDE9DB" },
  rowTitleDark: { color: "#16332F", fontSize: 13, fontWeight: "600" },
  rowSubDark: { color: "#6B6A60", fontSize: 11, marginTop: 2 },
  approveBtn: { backgroundColor: "#0F5C53", borderRadius: 8, paddingVertical: 6, paddingHorizontal: 10, marginRight: 6 },
});
